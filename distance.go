...
EditDistanceFunc and EditDistanceFuncFull are similar to EditDistance and EditDistanceFull, but using parameter and functions instead of an interface. This is sometimes more easy to use. ConstCost is a helper function.
*/
package Distance
...
var A_in = []byte{'A', 'R', 'M', 'W', 'D', 'H', 'V', 'N'} 
var C_in = []byte{'C', 'Y', 'M', 'S', 'B', 'H', 'V', 'N'} 
var G_in = []byte{'G', 'R', 'K', 'S', 'B', 'D', 'V', 'N'} 
var T_in = []byte{'T', 'Y', 'K', 'W', 'B', 'D', 'H', 'N'}

func base_cmp(a, b byte) bool {
	switch b {
	case 'A':
		if bytes.IndexByte(A_in, a) != -1 {
			return true
		}
		return false
	case 'C':
		if bytes.IndexByte(C_in, a) != -1 {
			return true
		}
		return false
	case 'G':
		if bytes.IndexByte(G_in, a) != -1 {
			return true
		}
		return false
	case 'T':
		if bytes.IndexByte(T_in, a) != -1 {
			return true
		}
		return false
	default:
...
/*
EditDistanceStr calculates the edit-distance between two strings. Input strings must be UTF-8 encoded.
...
/*
EditDistanceStrFull calculates the edit-distance and longest-common-string between two strings. Input strings must be UTF-8 encoded.
...
*/
func EditDistanceStrFull(a, b string) (dist int, lcs string) {
...
The time complexity is O(mn) where m and n are lengths of a and b, and space complexity is O(n).
*/
func EditDistanceFunc(lenA, lenB int, costOfChange func(iA, iB int) int, costOfDel func(iA int) int, costOfIns func(iB int) int) int {
	la, lb := lenA, lenB
...
/*
EditDistanceFuncFull returns the edit-distance and corresponding match indexes defined by parameters and functions.
Each element in matA and matB is the index in the other list, if it is equal to or greater than zero; or -1 meaning a deleting or inserting in matA or matB, respectively.
...
NOTE if detailed matching information is not necessary, call EditDistance instead because it needs much less memories.
*/
func EditDistanceFuncFull(lenA, lenB int, costOfChange func(iA, iB int) int, costOfDel func(iA int) int, costOfIns func(iB int) int) (dist int, matA, matB []int) {
	la, lb := lenA, lenB
